...
TIP: The command above assumes you have never before authorised any keys to access your Raspberry Pi. If you have previously added at least one key, you should instead add a new line containing the public key to the end of the `authorized_keys` file to preserve your existing keys.
...
If you want to limit SSH access to specific user accounts, edit the SSH server configuration:
...
Locate the `AllowUsers` directive and add the usernames you want to permit. If the line doesn't exist, add it to the bottom of the file:
...
You can also use `DenyUsers` to explicitly stop specific usernames from signing in:
