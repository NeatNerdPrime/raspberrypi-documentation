...
| `sysdefault:CARD=Headphones` | The headphone jack on Raspberry Pi 4 and earlier models
...
You can specify a video output using either the VLC GUI, or by using `--drm-vout-display` from the console.

To specify the video output from the console:

. Boot your Raspberry Pi device to the xref:../computers/configuration.adoc#console-or-desktop[console].
. Add the `--drm-vout-display` option:
+
[source,console]
----
...
+
| `HDMI-A-1` | The HDMI output on a Raspberry Pi Zero, or Raspberry Pi Model 1, 2 or 3; *or* the HDMI0 output on a Raspberry Pi 4B and later, and all Keyboard models
+
| `HDMI-A-2` | The HDMI1 output on Raspberry Pi 4B and later, Compute Module CM4 and later (excluding CM4S), and all Keyboard models
+
| `DSI-1` | The Raspberry Pi Touch Display, Raspberry Pi Touch Display 2, or third-party DSI screen
+
| `DSI-2` | The second DSI output for models with multiple DSI ports (Flagship models since Raspberry Pi 5, and all Compute Module models)
+
|===
...
NOTE: using `kmsprint` without parameters only queries the first card found. Therefore this might not list all attached displays on Raspberry Pi 5 and later models. See xref:../computers/os.adoc#kmsprint[kmsprint.]
