...
Raspberry Pi computers are equipped with a number of Serial Peripheral Interface (SPI) buses.

SPI can be used to connect a wide variety of peripherals – displays, network controllers (Ethernet, CAN bus), UARTs, and so on. These devices are best supported by kernel device drivers, but the `spidev` API allows userspace drivers to be written in a wide array of languages.
...
Raspberry Pi Zero, 1, 2 and 3 have three SPI buses:

* `SPI0`, with two hardware chip selects, is available on the header of all Raspberry Pi devices; there is also an alternate mapping that is only available on Compute Modules.
...
* `SPI3`, with two hardware chip selects.
* `SPI4`, with two hardware chip selects.
* `SPI5`, with two hardware chip selects.
* `SPI6`, with two hardware chip selects.

These SPI buses are available through alternate function assignments on certain GPIO pins.

For more information, see the https://pip.raspberrypi.com/documents/RP-008248-DS[BCM2711 Arm peripherals] datasheet.
...
On the Raspberry Pi 5, SPI functionality is provided by the xref:../computers/io-controllers.adoc[RP1 I/O controller] rather than the main application processor.

RP1 includes nine Synchronous Serial Interface (SSI) controllers. Six of these are routed to GPIO bank `0`. The remaining three - `SPI6`, `SPI7`, and `SPI8` - are internal to RP1 and are not routed to the standard 40-pin GPIO header.

* `SPI0`, with four hardware chip selects.
* `SPI1`, with three hardware chip selects.
* `SPI2`, with two hardware chip selects.
* `SPI3`, with two hardware chip selects.
* `SPI4`, with one hardware chip select.
* `SPI5`, with two hardware chip selects.

NOTE: DDR (Double Data Rate) operation is not supported on RP1 SPI controllers.
NOTE: `SPI4` and `SPI7` buses are device-mode only.
...
The LoSSI standard allows issuing of commands to peripherals (LCD) and to transfer data to and from them. LoSSI commands and parameters are 8 bits long, but an extra bit is used to indicate whether the byte is a command or parameter/data. This extra bit is set high for data and low for a command. The resulting 9-bit value is serialised to the output. LoSSI is commonly used with https://mipi.org/specifications/display-interface[MIPI DBI] type C compatible LCD controllers.
...
On Raspberry Pi 4 and earlier models, the default Linux driver is `spi-bcm2835`.

On Raspberry Pi 5, the RP1 SPI controllers use the Designware SPI/SSI Linux kernel driver `spi_dw`.
...
The driver supports all speeds which are even integer divisors of the core clock, although not all of these speeds support data transfer because of limits in the GPIOs and in the devices attached. As a rule of thumb, anything over 50 MHz is unlikely to work.
...
Bidirectional mode, also called 3-wire mode, is supported by the `spi-bcm2835` kernel module. In this mode, either the `tx` or `rx` field of the `spi_transfer` struct must be a NULL pointer, since only half-duplex communication is possible. Otherwise, the transfer will fail. The `spidev_test.c` source code does not consider this correctly, and therefore does not work at all in 3-wire mode.
...
`spidev` presents an `ioctl`-based userspace interface to individual SPI CS lines. Device Tree is used to indicate whether a CS line is going to be driven by a kernel driver module or managed by `spidev` on behalf of the user; it is not possible to do both at the same time. Raspberry Pi kernels are more relaxed about the use of Device Tree to enable `spidev`; the upstream kernels print warnings about such usage, and ultimately might prevent it altogether.
