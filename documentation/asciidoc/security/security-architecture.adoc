...
To implement secure boot and encryption, we recommend using our Secure Boot Provisioner, which can configure and deploy secure boot and disk encryption on multiple devices. For more information and instructions, see xref:../security/secure-boot-provisioner.adoc[Secure Boot Provisioner].
...
Disk encryption on Raspberry Pi devices protects the root filesystem by encrypting it with LUKS and binding the unlock key to the device using OTP, described in <<bootflow>>.
