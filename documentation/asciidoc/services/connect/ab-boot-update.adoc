...
Follow the three stages below to implement the A/B configuration and send updates using Raspberry Pi Connect:
...
. Stage 3: Use Raspberry Pi Connect's remote update feature to deploy the update artefact to the recipient device, then verify the update.
...
Now the device is configured for A/B booting, you can deploy update artefacts to it using Raspberry Pi Connect.

Update artefacts are created by rpi-image-gen as `.tar.zst` files. These are much smaller than the `.img` file you created in Stage 1, and are deployed in Stage 3 to the recipient device by Raspberry Pi Connect.
...
Deploy the artefact from the https://connect.raspberrypi.com/[Raspberry Pi Connect website], uploading the `.tar.zst` file you built in Stage 2, as described in <<update-remotely,Deploy the update artefact>>.

If the recipient device belongs to an organisation, host the artefact first, then deploy it using its URI and checksum, as described in xref:../services/connect-for-organisations.adoc#deploy-remote-updates[Deploy remote updates in an organisation].
