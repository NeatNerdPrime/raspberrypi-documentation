...
a|The recipient device must be able to locate the update file. Check that the device still has internet access.

For a device in an organisation, open a remote shell connection to the recipient device, then use `curl` to check whether the device can access the IP address contained in the URI you provided when creating the deployment.
...
a|Make sure that you deployed the `.tar.zst` update file, not the `.img` full image file. For a device in an organisation, check that the URI you provided when creating the deployment points at the `.tar.zst` file.

a|The deployment shows as **Invalidated**, with the reason `File expired`.
a|Connect keeps an artefact you upload for two hours, then deletes it. The recipient device didn't start the deployment before that happened. Check that the device is online, then deploy the artefact again.
...
For a device in your personal account, also check that the device is online: **Deploy** only appears while Connect can reach the device.
