...
- If connecting to the `DISP0` interface on Raspberry Pi Compute Module 4 IO Board.
...
NOTE: The 10-inch display requires four lanes of DSI data. On the CM5 Board and later, you can use either the `DISP0` or the `DISP1` port. On the CM4 Board and earlier, only port `DISP1` supports this.
...
You must configure the Compute Module for the devices it connects to by adding an overlay entry in `config.txt`.
...
+
. If using the `DISP0` port on a Compute Module 5, *add* the following:
+
...
. Slide the retaining clip outwards from both sides of the FFC connector on the Touch Display 2.
. Insert the FFC into the connector, then slide the retaining clip back into place to secure the cable.
+
The metal contacts must face upwards, away from the Touch Display 2.
...
+
The metal contacts must face downwards.
. If connecting to a Raspberry Pi Compute Module 5 IO Board `DISP1`, add two jumpers to `J6`.
