...
* The M.2 HAT+ includes threaded spacers that provide ample room to fit the Raspberry Pi Active Cooler beneath it. However, it's _only_ compatible with the https://www.raspberrypi.com/products/raspberry-pi-5-case/[Raspberry Pi Case for Raspberry Pi 5] _if you remove the lid and the included fan_.
...
. Firmly press the GPIO stacking header on top of the Raspberry Pi GPIO pins. The orientation doesn't matter, but the pins must all fit into place.
. Lift the ribbon cable holder gently from both sides on the M.2 HAT+ before disconnecting the ribbon cable.
. Lift the ribbon cable holder on the PCIe port of your Raspberry Pi from both sides, then insert the cable with the metal contact points facing towards the USB ports.
...
NOTE: If your NVMe drive is brand new or unformatted, it doesn't appear as an icon on the Raspberry Pi OS desktop. Additionally, commands such as `sudo blkid` don't show a partition UUID.  You must partition and format your drive to make it visible and accessible to the operating system. For more information, see <<boot-from-nvme,Boot from NVMe>> or xref:../computers/configuration.adoc#automatically-mount-a-storage-device[automatically mount a storage device].

. Connect your Raspberry Pi to power. Raspberry Pi OS automatically detects the M.2 HAT+ or M.2 HAT+ Compact.
+
* To verify the hardware is detected, open a terminal and run:
+
[source,console]
----
$ ls -l /dev/nvme*
----
+
If the drive has been correctly detected, you see `/dev/nvme0` and `/dev/nvme0n1`.

* If you plan to boot from the drive, see <<boot-from-nvme,Boot from NVMe>>. Raspberry Pi Imager automatically partitions and formats the drive during installation.
* If you plan to use the drive purely as additional storage, format the drive before xref:../computers/configuration.adoc#automatically-mount-a-storage-device[configuring automatic mounting].
...
. Flash Raspberry Pi OS to your NVMe drive using Raspberry Pi Imager. For more information, see xref:../computers/raspberry-pi.adoc[Installing Raspberry Pi OS].
* If you already have Raspberry Pi OS running from an SD card, run Raspberry Pi Imager directly on your Raspberry Pi and select the NVMe drive as the target storage.
* Alternatively, if your bootloader firmware is up to date, use the Raspberry Pi Network Installer to download and run Raspberry Pi Imager over an Ethernet connection without needing an SD card. For more information, see xref:../computers/raspberry-pi.adoc[Network installation].
...
For detailed information about NVMe boot, see xref:../computers/raspberry-pi.adoc[NVMe boot].

== Enable PCIe Gen 3

WARNING: The Raspberry Pi 5 is not certified for Gen 3.0 speeds. PCIe Gen 3.0 connections might be unstable.
